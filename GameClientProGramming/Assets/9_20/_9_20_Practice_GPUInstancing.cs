...
// 마우스 오른쪽 버튼 버튼 누를시 GPUInstancing  기능을 사용하는 게임오브젝트의 인스턴스 생성하고
//마우스로 게임오브젝트 선택시(마우스왼쪽버튼클릭) 선택한 게임오브젝트의 컬러를 검정색으로 변경하는 프로그램 작성

public class _9_20_Practice_GPUInstancing : MonoBehaviour
{
    GameObject rcTestObjerct;
    MaterialPropertyBlock tmpProps;
    private void Awake()
    {
        tmpProps = new MaterialPropertyBlock();
        rcTestObjerct = Resources.Load<GameObject>("Sphere");
    }
    ...
    public void Spawn()
    {
        GameObject createObject = GameObject.Instantiate<GameObject>(rcTestObjerct);
